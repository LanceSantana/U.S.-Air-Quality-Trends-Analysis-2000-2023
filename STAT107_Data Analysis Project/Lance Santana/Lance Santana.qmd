...
```{r}
#Sorts highest three ozone samples into a data frame
top_3_oz_samples <- pollution_data[order(pollution_data$O3.Mean, decreasing = TRUE),]
top_3_oz_samples <- top_3_oz_cities[1:3, c("State", "City", "O3.Mean")]
top_3_oz_samples

#Sorts lowest three ozone samples into a data frame
bottom_3_oz_samples <- pollution_data[order(pollution_data$O3.Mean, decreasing = FALSE),]
bottom_3_oz_samples <- bottom_3_oz_cities[1:3, c("State", "City", "O3.Mean")]
bottom_3_oz_samples
```
...
ggplot(washi_data, aes(x = O3.Mean)) +
  ...
  xlim(-.0006, 0.102) +
  labs(title = "Washington", x = "Ozone Samples")  +
  theme(
    axis.text.y = element_blank(),
    axis.ticks.y = element_blank()
  )
...
ggplot(oz_mean_by_state, aes(x = reorder(State, average_ozone_mean), y = average_ozone_mean)) +
  ...
  geom_hline(yintercept = median(oz_mean_by_state$average_ozone_mean), color = "red", linetype = "dashed", linewidth = 1) +
...
#Finds highest and lowest states by mean ozone
...
#Display previously found statistics
